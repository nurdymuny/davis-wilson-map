...
\item $\mathcal{C}_T$ is a set of \emph{transport constraints} (holonomy conditions)
\item $\mathcal{C}_P$ is a set of \emph{position constraints} (pointwise conditions)
\end{itemize}
The solution set is:
\[
S_{\text{valid}} = \{x \in M : x \text{ satisfies } \mathcal{C}_P\} 
\cap \{x : \text{holonomy at } x \text{ satisfies } \mathcal{C}_T\}
\]
Morphisms: Maps preserving both constraint types.
\end{definition}
