...
Let $\gamma_1, \ldots, \gamma_{b_{2p}}$ be loops generating $\pi_1(D)$.
The monodromy representation $\rho: \pi_1(D) \to GL(H^{2p}(X,\mathbb{C}))$ acts on cohomology.
For $\alpha \in H^{2p}(X,\mathbb{Q})$, we require:
\[
\|Hol_{\gamma_i}(\alpha) - \alpha\| = 0 \quad \forall i
\]
This is the constraint that $\alpha$ is monodromy-invariant over $\mathbb{Q}$.

\textbf{Constraint 2 (Type $(p,p)$):}
The Hodge filtration $F^p \subset H^{2p}(X,\mathbb{C})$ varies over $D$.
Let $\gamma_{F}$ be a loop around the discriminant locus.
Type $(p,p)$ means $\alpha \in F^p \cap \overline{F^p}$, which translates to:
\[
\|Hol_{\gamma_F}(\alpha) - \alpha\|_{F^p} < \tau_F
\]
where $\|\cdot\|_{F^p}$ measures deviation from the $F^p$ subspace.

\textbf{Constraint 3 (Hodge-Riemann):}
The Hodge-Riemann form $Q(\alpha, \bar{\alpha}) > 0$ for primitive classes.
Let $\gamma_{HR}$ be loops around the nilpotent cone boundary.
The constraint is:
\[
\|Hol_{\gamma_{HR}} - I\|_Q < \tau_{HR}
\]
where $\|\cdot\|_Q$ is the norm induced by the polarization $Q$.
\end{proof}
