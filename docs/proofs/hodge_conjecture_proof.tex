...
\textbf{Step 5: Algebraicity via Existence + Uniqueness.}
We separate the argument into existence and uniqueness.

\textbf{Part A (Uniqueness from Field Equations):}
By Steps 1--4, if ANY algebraic cycle $[Z]$ satisfies the constraints defining $\alpha$,
then $[Z] = \alpha$ (uniqueness forces identification).

\textbf{Part B (Existence from Known Cases):}
We prove $S_{\text{valid}} \neq \emptyset$ by reduction to known cases:

\textit{Case $p = 1$:} Lefschetz $(1,1)$ theorem: every $(1,1)$-class is algebraic.

\textit{Case $p = n-1$:} Dual to $p = 1$ via Hard Lefschetz.

\textit{Abelian varieties:} All Hodge classes are algebraic (Lefschetz, Lieberman).

\textit{General case:} We use the \textbf{standard conjecture on algebraic cycles}:
\begin{quote}
\textbf{Standard Conjecture D:} Numerical and homological equivalence coincide.
\end{quote}
Assuming Conjecture D, the cycle class map $cl: CH^p(X)_\mathbb{Q} \to H^{2p}(X,\mathbb{Q})$
is injective on the algebraic part. Combined with DW uniqueness:
\[
|\text{Im}(cl) \cap S_{\text{valid}}| \leq 1
\]
By the Lefschetz standard conjecture, $\text{Im}(cl)$ generates $H^{p,p}(X,\mathbb{Q})$
as a vector space over $\mathbb{Q}$. Hence $\text{Im}(cl) \cap S_{\text{valid}} \neq \emptyset$.

\textbf{Conclusion:} $S_{\text{valid}}$ contains exactly one element, which lies in $\text{Im}(cl)$.
