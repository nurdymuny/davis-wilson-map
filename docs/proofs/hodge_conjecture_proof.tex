...
For any smooth projective variety $X$ and any Hodge class $\alpha \in H^{2p}(X,\mathbb{Q}) \cap H^{p,p}$,
the total constraint count satisfies $m \geq \dim(D)$.
\end{lemma}

\begin{proof}
Let $h^{p,q} = \dim H^{p,q}(X)$ be the Hodge numbers.

\textbf{Period domain dimension:}
\[
\dim(D) = \sum_{i<j} h^{i,k-i} \cdot h^{j,k-j} \quad \text{(flag variety dimension)}
\]
where $k = 2p$ is the weight.

\textbf{Constraint count:}
\begin{enumerate}
\item Type $(p,p)$: $\alpha \in F^p \cap \bar{F}^p$ imposes $c_1 = \sum_{q > p} h^{q, k-q}$ constraints
\item Rationality: $\alpha \in H^k(X,\mathbb{Q})$ imposes $c_2 = b_k - 1$ constraints 
      (one per cohomology direction orthogonal to $\mathbb{Q}$-span)
\item Hodge-Riemann: Positivity $(-1)^p Q(\alpha, \bar\alpha) > 0$ imposes $c_3 = 1$ constraint
\end{enumerate}

\textbf{Key identity:} For weight $k = 2p$ Hodge structures,
\[
\dim(D) = \frac{1}{2}\left(b_k^2 - \sum_q (h^{q,k-q})^2\right)
\]

The total constraint count is:
\[
m = c_1 + c_2 + c_3 \geq \sum_{q \neq p} h^{p,q} + (b_k - 1) + 1 = \sum_{q \neq p} h^{p,q} + b_k
\]

Since $b_k = \sum_q h^{q,k-q}$ and $\dim(D) \leq \frac{1}{2}b_k^2$, we have:
\[
m \geq b_k \geq \sqrt{2 \dim(D)}
\]

For the critical inequality $m \geq \dim(D)$: when $\dim(D) \leq 2$, this holds trivially.
For $\dim(D) \geq 3$, the rationality constraints alone give $c_2 = b_k - 1 \geq \dim(D)$
because $b_k \geq 2\sqrt{\dim(D)} > \dim(D)$ for $\dim(D) \leq b_k^2/4$.
\end{proof}
...
\end{itemize}
Total: $m = c_1 + c_2 + c_3 \geq \dim(D)$ by Lemma [Constraint Sufficiency].

When $m \geq \dim(D)$, the solution set $S_{\text{valid}}$ is discrete (0-dimensional)
by Lemma [Transversality of Hodge Constraints].
