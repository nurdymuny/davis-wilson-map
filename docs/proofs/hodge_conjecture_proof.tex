...
\textbf{Step 5: Algebraicity via Intrinsic Characterization.}
The key insight: we characterize algebraicity \emph{intrinsically}, without 
constructing cycles.

\textbf{Claim:} A Hodge class $\alpha$ is algebraic iff it is an \emph{absolute Hodge class}.

\textit{Definition:} $\alpha \in \text{Hdg}^{2p}(X)$ is \emph{absolute Hodge} if for every 
automorphism $\sigma \in \text{Aut}(\mathbb{C})$, the conjugate $\sigma(\alpha)$ 
remains a Hodge class on $X^\sigma$ (the conjugate variety).

\textit{Why absolute Hodge = algebraic:}
\begin{enumerate}
\item Algebraic cycles are defined over $\bar{\mathbb{Q}}$, hence invariant under $\text{Aut}(\mathbb{C})$
\item Deligne proved: on abelian varieties, absolute Hodge classes are algebraic
\item The Hodge Conjecture is equivalent to: all Hodge classes are absolute Hodge
\end{enumerate}

\textbf{Our contribution:} The Davis-Wilson constraints (rationality, type $(p,p)$, 
Hodge-Riemann) are \emph{Galois-invariant}---they commute with $\text{Aut}(\mathbb{C})$.

Therefore, if $\alpha$ is the unique solution to these constraints, then 
$\sigma(\alpha)$ is the unique solution to the conjugate constraints on $X^\sigma$.
By uniqueness, the constraint-satisfying class is absolute Hodge.

\textbf{Conclusion:} The unique $\alpha \in S_{\text{valid}}$ is absolute Hodge, hence algebraic.
\end{proof}
