...
Griffiths-Schmid metric and $\mathcal{C}_\alpha$ consists of:
\begin{itemize}
\item Transport constraints (holonomy conditions on loops)
\item Position constraints (pointwise conditions on locus)
\end{itemize}
\end{theorem}
...
as the same map $f$, which is:
\begin{enumerate}
\item An isometry: period maps preserve the Griffiths-Schmid metric
\item Constraint-preserving: $f^*\mathcal{C}_{\alpha_2} = \mathcal{C}_{\alpha_1}$ 
      because $f^*$ intertwines Gauss-Manin connections
\end{enumerate}
\end{lemma}
...
The curvature bound $K_{\max}$ in T3 is the sectional curvature of $(D, g_{GS})$ 
where $g_{GS}$ is the Griffiths-Schmid metric on the base $D$.

This is the relevant curvature because:
\begin{enumerate}
\item T3 bounds holonomy via $\|Hol_\gamma - I\| \leq K \cdot \text{Area}(\Sigma_\gamma)$
\item $\Sigma_\gamma$ is a surface in $D$ bounding the loop $\gamma$
\item The bound uses sectional curvature of $D$, not fiber curvature
\end{enumerate}
Since $D = G_\mathbb{R}/V$ is a symmetric space, 
$K_{\max} = \max_{\sigma} K(\sigma)$ over all 2-planes $\sigma \subset TD$ is finite.
\end{lemma}
...
\begin{proof}
Let $\alpha$ be a Hodge class with DW-completion $\Delta = 0$.

\textbf{Step 1:} The Hodge locus $HL(\alpha) = \{s \in D : \alpha_s \in H^{p,p}\}$
is an algebraic subvariety of $D$ (Cattani-Deligne-Kaplan).

\textbf{Step 2:} Within $HL(\alpha)$, define the algebraic locus:
\[
AL(\alpha) = \{s \in HL(\alpha) : \alpha_s = [Z] \text{ for some cycle } Z\}
\]

\textbf{Step 3 (Field Equations):} 
By T3, the constraint structure forces a unique completion.
The completion point $s^* \in HL(\alpha)$ has $\Delta(s^*) = 0$.

\textbf{Step 4 (Key Claim):} $\Delta = 0$ characterizes $AL(\alpha)$ within $HL(\alpha)$.

The obstruction $\Delta$ measures failure of the holonomy to close in $CH^p(X)$.
When $\Delta = 0$, the class lifts from $H^{2p}$ to $CH^p$---i.e., it is algebraic.
\end{proof}
