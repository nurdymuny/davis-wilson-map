...
\begin{proof}
We do not need $m \geq \dim(D)$ globally. We need 
$m \geq \dim(HL(\alpha))$ where $HL(\alpha)$ is the Hodge locus.

The Hodge locus $HL(\alpha) \subset D$ is already constrained by the 
type $(p,p)$ condition. By Cattani-Deligne-Kaplan, $HL(\alpha)$ is a 
\emph{special subvariety} with:
\[
\dim(HL(\alpha)) \leq \dim(D) - \text{codim}(F^p \cap \bar{F}^p)
\]

Within $HL(\alpha)$, rationality imposes additional constraints. The key point:

\textbf{Claim:} Rationality + type $(p,p)$ implies $S_{\text{valid}}$ is discrete.

\textit{Proof of claim:} Monodromy acts on $H^{2p}(X,\mathbb{Z})$ preserving 
the integral lattice. Rational classes $\alpha \in H^{2p}(X,\mathbb{Q})$ form 
a discrete subset of the real cohomology $H^{2p}(X,\mathbb{R})$.

The monodromy orbit $\text{Mon} \cdot \alpha$ is a discrete set (finite orbits 
on lattice points). The type $(p,p)$ locus $HL(\alpha)$ is an algebraic subvariety.

The intersection:
\[
S_{\text{valid}} = HL(\alpha) \cap \{\text{monodromy-invariant classes}\}
\]
is the intersection of an algebraic variety with a discrete set, hence discrete.

More precisely: monodromy-invariance means $\alpha$ is fixed by $\pi_1(D)$-action.
The fixed locus of a discrete group acting on a lattice is a sub-lattice, 
which intersects any bounded region in finitely many points.
\end{proof}
