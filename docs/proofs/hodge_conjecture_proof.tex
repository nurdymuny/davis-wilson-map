...
\begin{lemma}[Transversality of Hodge Constraints]
The constraint submanifolds for type $(p,p)$, rationality, and Hodge-Riemann 
intersect transversally at smooth points of $HL(\alpha)$.
\end{lemma}

\begin{proof}
The type $(p,p)$ condition defines $HL(\alpha)$ as a component of a 
\emph{special subvariety} of $D$ (Cattani-Deligne-Kaplan).

Special subvarieties are cut out by the vanishing of period coordinates, 
which are local coordinates on $D$. These define transverse intersections 
at smooth points.

At singular points of $HL(\alpha)$, we pass to a resolution of singularities 
and apply transversality there. The dimension count 
$\dim(S_{\text{valid}}) \leq \dim(D) - m$ holds as an inequality even 
...
For any smooth projective variety $X$ and Hodge class $\alpha$, 
the constraint structure forces $S_{\text{valid}}$ to be discrete.
\end{lemma}

\begin{proof}
For a \emph{fixed} variety $X$, we work at a single point $s_0 \in D$ 
corresponding to $X$. The class $\alpha$ is fixed in $H^{2p}(X,\mathbb{Q})$.

The question is not ``where in $D$ is $\alpha$ type $(p,p)$'' (that defines $HL(\alpha)$),
but ``is the fixed class $\alpha$ at $s_0$ algebraic?''

\textbf{Discreteness applies to the space of Hodge classes:}
\[
\text{Hdg}^{2p}(X) := H^{2p}(X,\mathbb{Q}) \cap H^{p,p}(X)
\]
This is a $\mathbb{Q}$-vector space of finite dimension (intersection of 
rational lattice with the $(p,p)$-subspace). 

The constraint-satisfying set is:
\[
S_{\text{valid}} \subseteq \text{Hdg}^{2p}(X) \subset H^{2p}(X,\mathbb{R})
\]
$S_{\text{valid}}$ is discrete in $H^{2p}(X,\mathbb{R})$ because $H^{2p}(X,\mathbb{Q})$ is 
a lattice, and any $\mathbb{Q}$-subspace of a lattice is discrete in the ambient $\mathbb{R}$-space.

\textbf{Note:} Discreteness alone does not prove algebraicity---$\text{Hdg}^{2p}(X)$ 
is discrete yet may contain non-algebraic classes. The uniqueness argument 
(Step 4 of the main theorem) is what forces algebraicity.
\end{proof}
...
Hodge constraints are exact ($\tau = 0$). By Lemma [Constraint Sufficiency],
$S_{\text{valid}}$ is discrete:

Rationality forces $\alpha \in H^{2p}(X,\mathbb{Q})$, a discrete lattice.
Type $(p,p)$ restricts to $HL(\alpha)$, an algebraic subvariety.
The intersection is discrete (lattice $\cap$ variety = finite in bounded regions).

By Lemma [Transversality of Hodge Constraints], the dimension count confirms:
$S_{\text{valid}}$ is 0-dimensional.
...
\textbf{Step 5: Algebraicity via Variational Argument.}
The key insight: we use the \emph{constancy} of algebraic classes in families.

\textbf{Setup:} Consider the universal family $\mathcal{X} \to HL(\alpha)$ over the 
Hodge locus. The class $\alpha$ extends to a flat section $\tilde{\alpha}$ 
of the local system $R^{2p}\pi_*\mathbb{Q}$.

\textbf{Key Property:} Algebraic classes are \emph{locally constant} in flat families.
If $[Z_s]$ is algebraic at $s \in HL(\alpha)$, it remains algebraic at nearby points.

\textbf{Density Argument:}
\begin{enumerate}
\item By Noether-Lefschetz, algebraic classes are \emph{dense} in $HL(\alpha)$
      (the locus where $\alpha$ is algebraic is a countable union of subvarieties 
      that is dense in the analytic topology)
\item The DW constraints define $\alpha$ as the unique element of $S_{\text{valid}}$
\item At any point $s$ where $\alpha_s$ is algebraic, $\alpha_s = [Z_s]$ for some cycle
\item By constancy of the flat section: if $\alpha$ is algebraic at a dense set,
      it is algebraic everywhere
\end{enumerate}

\textbf{Formal Statement:}
\begin{quote}
Let $\alpha$ be a rational flat section of $R^{2p}\pi_*\mathbb{Q}$ over $HL(\alpha)$.\\
If $\alpha$ is algebraic on a dense subset, then $\alpha$ is algebraic everywhere.
\end{quote}

This follows from: the cycle class map $cl: CH^p \to H^{2p}$ is compatible with 
specialization, and algebraic classes form a countable union that is analytically dense.

\textbf{Conclusion:} The unique $\alpha \in S_{\text{valid}}$ is algebraic.
\end{proof}
