...
THE BUG: Left vs Right multiplication convention mismatch

Diagnostic showed 100% anti-alignment because:
- We were building W = link @ staple (U·A)
- But applying update on left: link = su3_update @ link (S·U)

The fix (per GPT analysis):
- Build W = staple @ link (A·U)  
- Apply update on right: link = link @ su3_update (U·S)
- Keep su2_new = R @ V† as is

Author: Bee Rosa Davis & Claude (with GPT debugging)
Date: December 2025
"""
...
def compute_staple(U: torch.Tensor, site: Tuple[int, ...], mu: int) -> torch.Tensor:
    """Original staple computation (keep as-is per GPT)."""
    device = U.device
    dtype = U.dtype
    dims = U.shape[:4]
...
def cabibbo_marinari_update(U: torch.Tensor, site: Tuple[int, ...], mu: int, 
    ...
    """
    Cabibbo-Marinari update - FIXED left/right convention.
    
    THE FIX (per GPT):
    - W = staple @ link (was: link @ staple)
    - link = link @ su3_update (was: su3_update @ link)
    - Keep su2_new = R @ V† as is
    """
    device = U.device
    t, x, y, z = site
    ...
    for subgroup in range(3):
        # ============================================================
        # FIX 1: W = staple @ link (was: link @ staple)
        # ============================================================
        W = staple @ link
        ...
        if k < 1e-8:
            ...
            # Keep this as is (per GPT)
            su2_new = R @ V.conj().T
        
        su3_update = embed_su2_in_su3(su2_new, subgroup, device)
        
        # ============================================================
        # FIX 2: link = link @ su3_update (was: su3_update @ link)
        # ============================================================
        link = link @ su3_update
...
def main():
    ...
    print("  Fix 1: W = staple @ link (was link @ staple)")
    print("  Fix 2: link = link @ su3_update (was su3_update @ link)")
    print("=" * 60)
    ...
    if 0.55 < final_plaq < 0.65:
        ...
        print("❌ STILL getting ~0.25-0.30 - need to check subgroup mapping")
    else:
