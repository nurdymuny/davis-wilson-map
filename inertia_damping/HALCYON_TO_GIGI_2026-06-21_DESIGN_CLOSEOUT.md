...
1. ✅ **Zenodo deposit of v3.1.3 complete.** DOI [10.5281/zenodo.20785681](https://doi.org/10.5281/zenodo.20785681) minted 2026-06-21 with v3.0, v3.1, v3.1.1, v3.1.2 attached for chain-of-custody transparency. The DOI is the publication-level pre-registration; the canonical SPEC commit `44c70b1` is tagged `spec-v3.1.3-zenodo-20785681` in git. The post-deposit single-line commit that adds the `Zenodo DOI:` pointer to the top of `HALCYON_FALSIFICATION_BATTERY_SPEC_v3.1.3.md` is *not* part of the pre-registration — it merely records the DOI for cross-reference. From here, §3 falsification criteria are locked at the deposited state; any further amendment would be a v3.1.4 with its own pre-registration timestamp.
