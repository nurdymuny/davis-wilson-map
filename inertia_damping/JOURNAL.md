...
## 2026-06-22 — The v3.1.3 publication-bound run: bivalent AMBIGUOUS verdict + named ambiguity

### The shape of the entry

The publication-bound run completed at 07:37 UTC. Yesterday's Zenodo deposit at `10.5281/zenodo.20785681` against SPEC commit `44c70b1` committed the project to publishing whatever verdict the protocol returned; today's verdict at both pre-registered calibrations is AMBIGUOUS, driven by substrate-gate failures rather than by signal strength. Under v3.1.3 §3.3 the framework status is *not yet falsified*; under §3.7 a re-run is permitted with named ambiguity, more seeds or longer T_segment (or both, if the diagnosis identifies both as needed), no protocol parameter changes, and an amendment committed before the re-run. This JOURNAL entry is the Halcyon-side amendment.

Wall-clock per the sidecar file mtimes: α=1 sidecar written at 07:30:03 UTC, α=1000 sidecar written at 07:37:07 UTC — α=1000 took approximately 7 minutes wall once α=1 completed. Substrate: `gigi-stream` built from gigi commit `ccc039e` (post-Option-A, post-VI.6b). Orchestrator: davis-wilson-lattice commit `b0458bd`. Configuration: loop `gamma_unit_in_Q_beta_W` per v3.1.3 §4.4, lattice `halcyon_canonical_buckyball` (truncated icosahedron, SU(2)), eight seeds (`20260616`..`20260623`), per-seed orchestrator-side thermalization per the `5add5da` decomposition, $N_\text{DISCRETIZATION} = 10000$, dual α calibrations $\alpha_\text{Halcyon} = 1.0$ and $\alpha_\text{Halcyon} = 1000.0$ per v3.1.3 §3.6. Sidecar artifacts at `inertia_damping/reports/holonomy_battery_v3_1_3/section_12_holonomy_battery_alpha_1.json` (5895 bytes) and `..._alpha_1000.json` (5851 bytes) — both committed alongside this entry.

### The verdict in detail

Band definitions per v3.1.3 §3.5: NULL when $|H_\text{geom}|/\sigma_H < 1$, AMBIGUOUS when $1 \leq |H_\text{geom}|/\sigma_H < 5$, POSITIVE when $|H_\text{geom}|/\sigma_H \geq 5$ conditioned on sign coherence $\geq 5/8$. The substrate gates per §4.2 fire first regardless of band assignment.

| Quantity | $\alpha = 1.0$ | $\alpha = 1000.0$ |
|---|---|---|
| Verdict | AMBIGUOUS | AMBIGUOUS |
| $H_\text{geom mean}$ | $-2.105 \times 10^{-2}$ | $+1.227 \times 10^{-1}$ |
| $\sigma_{H,\text{blocked}}$ | $9.685 \times 10^{-2}$ | $1.168 \times 10^{-1}$ |
| $|H_\text{geom}| / \sigma$ | $0.22$ (NULL band per §3.5) | $1.05$ (borderline AMBIGUOUS band per §3.5) |
| $H_\text{sys}$ | $-1.360 \times 10^{-1}$ | $+6.604 \times 10^{-5}$ |
| Sign coherence | $5/8$ (meets POSITIVE requirement per §3.5) | $6/8$ |
| Sham battery | $5/5$ FAILED (FLAT_FIELD, ALPHA_ZERO, MASS_SCALED, BACKTRACK_LOOP, FROZEN_FIELD) | $5/5$ FAILED |
| `tracking_error_max_Q` | $0.115 \geq \varepsilon_Q = 0.05$ — FAIL | $0.132 \geq \varepsilon_Q = 0.05$ — FAIL |
| `tracking_error_max_β_W` | $0.232 \geq \varepsilon_{\beta_W} = 0.05$ — FAIL | $0.279 \geq \varepsilon_{\beta_W} = 0.05$ — FAIL |
| $\tau_\text{pin} / T_\text{segment}$ | $1.0 \times 10^{12}$ (adiabaticity gate per §4.2 fired) | $1.0 \times 10^{9}$ (adiabaticity gate fired) |

The verdict at both calibrations is AMBIGUOUS *driven by substrate-gate failures*, not by signal strength. Reading the underlying signal as if the substrate gates had passed: at $\alpha = 1$ the signal magnitude $|H|/\sigma = 0.22$ sits well inside the NULL band per §3.5; at $\alpha = 1000$ the signal magnitude $1.05$ sits at the lower edge of the AMBIGUOUS band per §3.5. The composite classifier per §3 + §4 returns AMBIGUOUS regardless because the substrate gates fire first — once `tracking_error_max` exceeds the pre-registered $\varepsilon$ on either axis, or once $\tau_\text{pin}/T_\text{segment} \geq 0.1$, the verdict cannot reach POSITIVE no matter what the signal does.

### Named ambiguity — what the substrate gates actually told us

Three substrate-side behaviors fired the AMBIGUOUS verdict. Each one has a likely diagnosis at the measurement-formula level that the §3.7 re-run amendment will need to either confirm or rule out before the protocol fires again.

**1. $\tau_\text{pin}/T_\text{segment}$ at the clamp floor.** The substrate's `τ_pin` formula computes $T_\text{segment} / \text{Gauss residual}$. The Gauss residual has a clamp floor at $10^{-12}$ — the same clamp Finding #3 surfaced against the identity-U state yesterday afternoon (JOURNAL `2026-06-21 — Cross-team audit trail`, VI.6b receipts). When the Gauss residual decays to the clamp floor, $\tau_\text{pin}/T_\text{segment} = T_\text{segment} \cdot 10^{12} / T_\text{segment} = 10^{12}$ at $\alpha = 1$ and $10^{9}$ at $\alpha = 1000$. The pinning *is* adiabatic — Gauss is conserved at machine precision, which is what the SPEC actually wants — but the measurement-formula ratio doesn't behave well at the clamp floor and the §4.2 gate fires on what is almost certainly a measurement-formula artifact rather than physically non-adiabatic dynamics. This is the same shape as the identity-U $\tau_\text{pin} = 10^{12}$ result from the VI.6b receipts entry, only now showing up against a thermalized loop instead of an identity field.

**2. `tracking_error_max` above $\varepsilon$ on both axes at both α.** The $Q$-axis tracking error sits at $0.115$ ($\alpha=1$) / $0.132$ ($\alpha=1000$); the $\beta_W$-axis at $0.232$ / $0.279$. Both axes' pre-registered thresholds are $\varepsilon = 0.05$. This is the more interesting of the three failures because it could be genuinely non-adiabatic — the parameter-space ramp rate at the chosen pin λ may be too fast for the substrate's surrogate to track — or it could be measurement-formula related in the same family as the τ_pin clamp issue. The diagnostic question is whether the tracking error reflects real ramp-rate / pin-λ tension or whether it is also pinning to a substrate-side numerical artifact. The VI.6b receipts from yesterday recorded $Q$-axis tracking error $0.079$ and $\beta_W$-axis $0.151$ on the diagnostic state at $\alpha = 1$; today's run gives $0.115$ and $0.232$ on the publication-bound battery state. The values *grew* on the live loop relative to the diagnostic fire, which is at least consistent with the substrate's surrogate having a harder time tracking the actual battery dynamics than the diagnostic conditions.

**3. Five of five shams failed.** All five §3.6 sham controls returned non-zero outputs that exceed $\varepsilon_\text{abs} = 1 \times 10^{-10}$. FLAT_FIELD at $\alpha = 1$ produced $0.168$ — six orders of magnitude above the threshold. Either VI.4's SHAM-flag dispatch is not fully nulling the signal at the substrate level (a flag-handling defect post-Option-A), or the new signed-arccos reduction from Fix #1 changed what "near-zero holonomy" looks like for the FLAT_FIELD test — under signed arccos a zero coupling field may not produce identically-zero output through the projection if there is any numerical asymmetry in the path-ordering reduction. A third candidate: the pre-registered $\varepsilon_\text{abs} = 10^{-10}$ may itself be tighter than the signed-arccos observable's noise floor, in which case the threshold wants adjustment via amendment rather than the substrate. Note BACKTRACK_LOOP is a different failure shape from the other four — it returns mean_h = 0.0, σ = 0.0 across all 8 seeds and then fails the comparator $|\text{mean}| \geq 2\sigma$ degenerately ($0 \geq 0$). That looks like a sham-gate logic issue (divide-by-zero or degenerate-comparison handling) distinct from FLAT_FIELD / ALPHA_ZERO / FROZEN_FIELD, which are non-nulling at the 0.17–0.43 level.

### Pre-registration discipline doing its job

Four properties of today's run are exactly what the v3.1.3 design phase was supposed to produce, and each one held.

The signed-arccos primary observable is alive at the substrate level: $H_\text{geom mean}$ is non-zero at both calibrations ($-0.021$ at $\alpha=1$, $+0.123$ at $\alpha=1000$), which is the antisymmetric reduction that v3.1.3 §3.1 calls for. The pre-Option-A scalar definition that the v3.0 review chain caught as identically zero by the fundamental theorem of calculus would not have produced these values — both calibrations would have returned $H \approx 0$ for reasons unrelated to physics, and the verdict would be a noise-floor question rather than a substrate-gate question. The pre-deposit review chain rewrote §3.1 against exactly this failure mode and today's run confirms the rewrite landed.

The per-seed orchestrator-side thermalization is producing real ensemble variance: $\sigma_{H,\text{blocked}} = 0.097$ at $\alpha=1$ and $0.117$ at $\alpha=1000$. These are not single-configuration measurement noise; the `5add5da` decomposition is doing what it was designed to do — eight seeds give eight genuinely independent forward/reversed pairs and the spread reflects per-seed gauge-configuration variance. The CSPRNG-determinism sentinel from yesterday's audit-trail entry was a precondition for this to be trustworthy; today's $\sigma$ values are the receipt that the cross-team determinism contract held end-to-end.

The substrate gates fired when they were supposed to. The v3.1.3 §4.2 wording — *"the substrate's ADIABATICITY_CHECK is required to verify $\tau_\text{pin} \ll T_\text{segment}$ at runtime. A violation forces AMBIGUOUS regardless of the H values"* — was written into the SPEC against exactly the scenario where the underlying signal might look like a result but the substrate cannot vouch for adiabaticity. The gate fired on a measurement-formula failure today and the verdict respected the SPEC. The discipline that wrote the gate is the same discipline that respects it now; if the gate had been advisory rather than load-bearing, today's run would arguably report POSITIVE at $\alpha=1000$ on a $1\sigma$ signal that the substrate cannot certify against the SPEC's adiabaticity requirement.

The §3.7 re-run criteria handle this case explicitly. The pre-deposit review chain anticipated AMBIGUOUS outcomes specifically *because* the publication commitment forecloses the option of running until POSITIVE; the §3.7 re-run pathway requires named ambiguity (recorded above), more seeds or longer $T_\text{segment}$ (or both, if the substrate-side diagnosis identifies both as needed independently), no protocol parameter changes (pin λ, ramp rates, $\varepsilon_Q$, $\varepsilon_{\beta_W}$, $\varepsilon_\text{abs}$, adiabaticity threshold $0.1$, sham threshold, sign-coherence requirement, primary band cutoffs all stay), and an amendment committed before the re-run. This JOURNAL entry is the amendment for the named-ambiguity item; the substrate-side amendment naming the chosen remediation is on Gigi's side and lands before any re-run fires.

For the avoidance of doubt: even if all three substrate-side observations (τ_pin clamp-floor reading, tracking_error above $\varepsilon$, SHAM dispatch non-zero) are diagnosed as measurement-formula artifacts at the substrate level and the substrate gates would have passed at the re-run, the underlying signal at $\alpha=1$ ($|H|/\sigma=0.22$) sits in the NULL band and the underlying signal at $\alpha=1000$ ($|H|/\sigma=1.05$) sits at the lower edge of the AMBIGUOUS band per §3.5. Neither calibration's signal alone could produce a POSITIVE verdict at the §3.5 band definition; the §3.7 re-run, if it fires, is still required to resolve the borderline $\alpha=1000$ reading, and the resolution might still be NULL or AMBIGUOUS rather than POSITIVE. A POSITIVE outcome at re-run requires the signal *itself* to cross the $5\sigma$ band — substrate-side fixes alone do not get us there.

### What §3.7 requires for a re-run

Itemized so neither side has to re-derive the requirements later. Each box has to be checked before the next run fires; if any one is missing, §3.7 prohibits the re-run and the AMBIGUOUS verdict is published as the terminal result on this protocol revision.

- **Named ambiguity** — recorded above (τ_pin at clamp floor, tracking_error above $\varepsilon$ on both axes, $5/5$ shams produce non-zero with BACKTRACK_LOOP a distinct degenerate-comparator failure).
- **More seeds or longer $T_\text{segment}$** — at least one of the two; the substrate-side investigation determines which (or both, if the diagnosis identifies both as needed independently). §3.7 as written permits either knob individually or in combination; it forbids only changes to the protocol parameters themselves (pin λ, ramp rates, ε thresholds, ε_abs, adiabaticity threshold, sham threshold, sign-coherence requirement, primary band cutoffs).
- **No protocol parameter changes** — see above; all the listed parameters stay verbatim from v3.1.3.
- **Amendment committed before re-run** — this JOURNAL entry covers the Halcyon side; a substrate-side amendment naming the remediation for the τ_pin clamp-floor reading + tracking-error diagnosis + SHAM dispatch fix is required from Gigi's side before the re-run.

### The honest physics reading

Setting the substrate-gate failures aside and reading the underlying signal as if all the gates had passed: this is not a discovery. At $\alpha = 1$ the signal magnitude $0.22$ is well below the $1\sigma$ NULL boundary; the most defensible reading is that the Halcyon coupling, at this protocol's signal-to-noise floor and on this lattice at $\alpha = 1$, is consistent with zero. At $\alpha = 1000$ the signal magnitude $1.05$ is borderline; if the substrate gates had passed, $\alpha = 1000$ would land in the AMBIGUOUS band by signal alone and §3.7 re-run with longer $T_\text{segment}$ or more seeds would be the next step regardless of gate behavior.

The bivalent reading across both α: the Halcyon coupling is not visible at this protocol's signal-to-noise floor against the buckyball SU(2) substrate at either calibration. Whether that null is "the coupling is not real" or "the coupling is real but the buckyball lattice is the wrong substrate to see it on" or "the coupling is real but the chosen loop area, pin λ, and ramp rate are not where it shows" — none of those readings can be distinguished by today's run. The §3.7 re-run, if it fires, narrows one of those questions; it does not by itself give the answer the protocol was designed to ask.

### What unblocks next

The cross-team investigation queue is:

- **τ_pin formula behavior at the Gauss-residual clamp floor** — is the $10^{12}$ value really "non-adiabatic" in the physical sense the §4.2 gate was meant to catch, or is it a measurement-formula artifact where the gate fires on the clamp's reciprocal? Diagnosis question for the substrate side; if it is the latter, the remediation is either to raise the clamp floor enough that the ratio normalizes to the $(0, 100)$ adiabaticity bracket the SPEC describes, or to redefine the substrate's emitted quantity so it reads as a true adiabaticity diagnostic at the clamp-saturated regime.
- **`tracking_error_max` above $\varepsilon$ on both axes at both α** — genuinely non-adiabatic at the chosen pin λ + ramp rates, or substrate measurement-formula related? Diagnosis distinguishes whether a longer $T_\text{segment}$ would close the gate (the ramp-rate reading) or whether a clamp-floor or projection-arithmetic fix would (the measurement-formula reading).
- **SHAM dispatch returning non-zero at machine precision** — FLAT_FIELD = $0.168$ at $\alpha = 1$ is the loudest of the five; substrate-side question on whether the SHAM flag is fully nulling the coupling post-Option-A, or whether the signed-arccos reduction interacts with the SHAM zeroing in a way that produces residual non-zero on what should be an identically zero output. BACKTRACK_LOOP's degenerate $0/0$ failure is a separate sham-gate logic issue.
- **Third diagnostic possibility worth naming explicitly**: the substrate-side investigation might conclude none of the above — that the protocol genuinely does not have signal at these calibrations against this substrate, i.e., neither the clamp-floor reading nor the ramp-rate reading explains the gate failures and the underlying physics is that the Halcyon coupling is not visible at this protocol's signal-to-noise floor on the buckyball SU(2) lattice at the dual α calibrations. That outcome still produces AMBIGUOUS at re-run but for a third reason and would inform the design of any v3.2 protocol revision: different lattice, different loop area, different pin λ, or coarser α calibrations. The §3.7 re-run is permitted regardless of which of these diagnoses returns; only protocol-parameter changes are forbidden.

Once the diagnoses land, the substrate-side amendment is committed, then the §3.7 re-run fires at whichever knob(s) the diagnoses identify. Until then, the AMBIGUOUS verdict is the published result.

### Ownership

Halcyon side: complete for v3.1.3. The published verdict (this JOURNAL entry plus the two sidecar JSONs at `inertia_damping/reports/holonomy_battery_v3_1_3/section_12_holonomy_battery_alpha_1.json` and `..._alpha_1000.json`) is the result the Zenodo-deposited SPEC commits the project to publishing regardless of direction. The book chapter for Solves Vol. 4 lifts from this entry: published verdict AMBIGUOUS at both calibrations, named ambiguity (substrate-gate failures: τ_pin at clamp floor, tracking_error above $\varepsilon$ on both axes, $5/5$ shams produce non-zero with BACKTRACK_LOOP a distinct degenerate-comparator shape), §3.7 re-run plan (amendment committed before re-run, substrate-side issues named, knob(s) chosen by substrate-side diagnosis), Halcyon coupling status *not yet falsified* per v3.1.3 §3.3. Halcyon is awaiting cross-team review of the three substrate-gate behaviors before any §3.7 re-run is fired.

Gigi's queue: substrate-side investigation of the three named-ambiguity items above (τ_pin formula at the clamp floor, tracking_error diagnosis, SHAM dispatch returning non-zero). When the investigation lands with a chosen remediation and the substrate-side amendment is committed, the §3.7 re-run can fire — one shell invocation against the rebuilt `gigi-stream.exe`, no Halcyon-side change required between now and then.
